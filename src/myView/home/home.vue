...
<template>
  ...
  <div>
    <Row :gutter="40"
         type="flex"
         justify="center"
         v-for="(row) in listTemp"
         :key="row">
      <i-col :xs="12"
             :md="8"
             :lg="4"
             v-for="(infor,j) in row"
             :key="`infor-${j}`"
        ...
        <infor-card shadow
          ...
          <count-to :end="infor.count"
                    count-class="count-style"></count-to>
          <p>{{ infor.title }}</p>
        </infor-card>
      </i-col>
    </Row>
    ...
    <Scroll :on-reach-bottom="handleReachTop1"
            loading-text="已加载全部"
            style="width: 380px;margin-left:215px">
    ...
    <Scroll :on-reach-bottom="handleReachTop2"
            loading-text="已加载全部"
            style="width: 380px;margin-left: 635px; margin-top:-300px">
    ...
    statisticAllData().then((res) => {
      ...
      if (res.data.success) {
        this.inforCardData[0].count =
          res.data.result.statisticDto.superAdminCount
